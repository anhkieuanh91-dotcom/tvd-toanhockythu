...
function generateGeometryQuestion(gradeId: number, levelId: string, type: QuestionType, index: number, language: string): Question {
  ...
  // Adapt to types
  if (type === 'matching') {
    // Filter out questions that strictly require an image for matching
    const suitableQs = gradeQs.filter(q => {
      const text = q.text.toLowerCase();
      if (text.includes('hình bên') || text.includes('hình dưới') || text.includes('hình này')) {
        // Keep only if it's a general property question that doesn't strictly need the specific image
        return text.includes('có mấy cạnh') || text.includes('có bao nhiêu đỉnh') || text.includes('có mấy góc vuông');
      }
      return true;
    });

    // Pick 4 random questions from the suitable list to form pairs with unique answers
    const shuffledQs = [...suitableQs].sort(() => Math.random() - 0.5);
    const selectedQs = [];
    const usedAnswers = new Set<string>();
    ...
    const pairs = selectedQs.map(sq => {
      let sqText = language === 'en' ? sq.textEn : language === 'vi-en' ? `${sq.text} / ${sq.textEn}` : sq.text;
      
      // Clean up text if it still contains "What shape is this?" or "Hình bên dưới là hình gì?"
      if (sqText.includes('Hình bên dưới là hình gì?') || sqText.includes('What shape is this?')) {
        sqText = language === 'en' ? sq.textEn.replace('What shape is this?', 'Shape with feature: ' + sq.explEn) :
